...
def pitch_onset_features_to_DLNCO(f_peaks: dict,
    ...
    for midi_pitch in range(midi_min, midi_max + 1):
        if midi_pitch not in f_peaks:
            continue
        time_peaks = np.asarray(f_peaks[midi_pitch][0, :], dtype=np.float64).ravel() / 1000  # Now given in seconds
        val_peaks = np.log(np.asarray(f_peaks[midi_pitch][1, :], dtype=np.float64).ravel() * log_compression_gamma + 1)
        ind_chroma = np.mod(midi_pitch, 12)
        for k in range(time_peaks.size):
            indTime = __matlab_round(float(time_peaks[k]) * feature_rate)  # Usage of "round" accounts
